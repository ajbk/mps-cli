...
3. Create or update one canonical character sheet before generating a deck. The sheet must show the requested hair, glasses, white outfit, and front/side/back proportions. Set `character.status` to `needs-review` until the teacher approves it.
...
6. Keep new cards at `draft` or `needs-review` until the image passes a human review for identity, proportions, outfit, anatomy, apparatus, and exercise setup. Only set `character.status` to `approved` after the canonical sheet is accepted; only then may cards become `approved`.
...
- Preserve the shoulder-length layered bob, round dark glasses, and white Pilates top/bottom unless the manifest explicitly changes the approved variant.
